...
struct NormalMapUniformBufferObject {
    alignas(16) glm::mat4 mvpMat;
...
// **A10** Place here the variable for the DescriptorSetLayout
    DescriptorSetLayout DSLNormalMap;
...
// **A10** Place here the variables for the Model, the five texture (diffuse, specular, normal map, emission and clouds) and the Descrptor Set
    Model Mearth;
    Texture TearthDiff, TearthSpec, TearthNM, TearthEmit, TearthClouds;
...
// **A10** Place here the initialization of the the DescriptorSetLayout
        DSLNormalMap.init(this, {
                    {0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_VERTEX_BIT, sizeof(NormalMapUniformBufferObject), 1},
                    {1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 0, 1},
                    {2, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 1, 1},
...
// **A10** Place here the initialization for the VertexDescriptor
        VDNormalMap.init(this, {
                    {0, sizeof(earthVetex), VK_VERTEX_INPUT_RATE_VERTEX}
                  }, {
                    {0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(earthVetex, pos),
                           sizeof(glm::vec3), POSITION},
                    {0, 1, VK_FORMAT_R32G32B32_SFLOAT, offsetof(earthVetex, norm),
                           sizeof(glm::vec3), NORMAL},
                    {0, 2, VK_FORMAT_R32G32_SFLOAT, offsetof(earthVetex, UV),
                           sizeof(glm::vec2), UV},
                    {0, 3, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(earthVetex, tangent),
...
//		And should receive two DescriptorSetLayout, the first should be DSLGlobal, while the other must be the one you defined
        PNormalMap.init(this, &VDNormalMap, "shaders/NormalMapVert.spv", "shaders/NormalMapFrag.spv", {&DSLGlobal, &DSLNormalMap});
...
//		Vertex descriptor you defined, and be of GLTF format.
        Mearth.init(this, &VDNormalMap, "models/Sphere.gltf", GLTF);
...
// **A10** Place here the loading of the four textures
        ...
        TearthNM.init(this, "textures/2k_earth_normal_map.png", VK_FORMAT_R8G8B8A8_UNORM);
		// Emission map in: "2k_earth_nightmap.jpg"
        TearthEmit.init(this, "textures/2k_earth_nightmap.jpg");
...
// Textures should be passed in the diffuse, specular, normal map, emission and clouds order.
        DSearth.init(this, &DSLNormalMap, {&TearthDiff, &TearthSpec, &TearthNM, &TearthEmit, &TearthClouds});
...
// **A10** Add the commands to bind the pipeline, the mesh its two descriptor setes, and the draw call of the planet
        PNormalMap.bind(commandBuffer);
        Mearth.bind(commandBuffer);
        DSGlobal.bind(commandBuffer, PNormalMap, 0, currentImage);
        DSearth.bind(commandBuffer, PNormalMap, 1, currentImage);
        vkCmdDrawIndexed(commandBuffer,
...
// **A10** Add to compute the uniforms and pass them to the shaders. You need two uniforms: one for the matrices, and the other for the material parameters.
        ...
        NormalMapUniformBufferObject nmubo{};
        nmubo.mvpMat = ViewPrj;
        nmubo.mMat = glm::mat4(1.0f);
        nmubo.nMat = glm::mat4(1.0f);
        DSearth.map(currentImage, &nmubo, 0);
        ...
        NormalMapParUniformBufferObject nmpubo{};
        nmpubo.pow = 200.0;
        ...
        nmpubo.ang = tTime * TangTurnTimeFact;
